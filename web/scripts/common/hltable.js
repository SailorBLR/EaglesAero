	...
	if (hoverClass)
	{
		...
		table.onmouseover = table.onmouseout = function(e)
		{
			...
			if (elem.parentNode.tagName == 'TR' && elem.parentNode.parentNode.tagName == 'TBODY')
			{
				...
				//Если текущий ряд не "кликнутый" ряд, то в разисимости от события либо применяем стиль, назначая класс,
				// либо убираем.
				if (!row.getAttribute('clickedRow')) row.className = e.type=="mouseover"?row
						.className+" "+hoverClass:row.className.replace(hoverClassReg," ");
			}
		};
	}
